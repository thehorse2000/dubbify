...
Transcription: Extract the spoken words from a source media file (.mp4, .mov, .mp3, .wav) and generate a timed subtitle file (.srt). This is powered by ElevenLabs.
...
Choice of Transcriber: ElevenLabs-only for transcription.
...
Configuration:
Users must set their API key as environment variables.
...
# It is recommended to load API key from environment variables
...
Checks if media_path is video; if so, calls media_utils.extract_audio.
    ...
    Calls ElevenLabs STT endpoint and formats the response into standard SRT.
...
Implement the Transcriber class using ElevenLabs STT.
